...
class Permission extends AdminController{
	...
	public function search() {
		...
		foreach($filteredData as $result) {
			...
			$action .= 		'<a class="btn btn-sm btn-info" href="'.admin_url('permission/edit/'.$result->id).'"><i class="fas fa-pencil-alt"></i></a>';
			$action .=		'<a class="btn-sm btn btn-danger btn-remove" href="'.admin_url('permission/delete/'.$result->id).'" onclick="return confirm(\'Are you sure?\') ? true : false;"><i class="fas fa-trash"></i></a>';
			$action .= '</div>';
	...
	public function assign(){
        ...
        if ($this->request->getMethod(1) === 'POST'){
			...
			$users=(new UserModel())->where('user_role_id',$id)->findAll();
			foreach($users as $user){
				$this->permissionModel->addUserPermission($user->id,$this->request->getPost());
			}
            $this->session->setFlashdata('message', 'Permission Updated Successfully.');
        ...
        if((int)$id) {
            $roles_info = $userrolemodel->find($id);
            $data['text_form'] = $roles_info->name ." Permission";
