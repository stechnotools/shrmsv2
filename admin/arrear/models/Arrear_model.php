 ...
 */
class Arrear_model extends MY_Model
{	
   public function __construct(){
		parent::__construct();
	}
	public function addArrear($data) {
		//printr($_POST);
		//exit;
		$arreardata=array(
      	...
      	$this->db->insert("employee_payroll", $arreardata);
      	$arrear_id=$this->db->insert_id() ;
		return $arrear_id;
	}
	public function editArrear($arrear_id, $data) {
		$arreardata=array(
			"ctc"=>$data['ctc'],
			"basic"=>$data['basic'],
			"da"=>$data['da'],
		...
		$this->db->where("user_id",$arrear_id);
      	$this->db->update("employee_payroll", $arreardata);
		
		
	}
	public function getArrears($data = array()){
	...
	public function getTotalArrears($data = array()) {
		$this->db->from("employee_payroll ep");
		$this->db->join('employee_office eo', 'eo.user_id = ep.user_id','left');
	...
	public function deleteArrear($user_id){
		$this->db->where_in("user_id", $user_id);
		$this->db->delete("employee_payroll");
	}
	
	public function getArrearSheets($data = array()){
		...
		if (!empty($data['arrear_mode'])) {
			$this->db->where("
				ep.arrear_mode = '{$data['arrear_mode']}'"
			);
		}
	...
	public function getArrearSheet($user_id,$month){
	...
	public function getSiteAttendanceArrears($attendance_id,$designation_id) {
