...
class Salary extends Admin_Controller {
	...
	protected function getForm(){
		...
		if ($id && ($this->input->server('REQUEST_METHOD') != 'POST')) {
			$emp_payroll = $this->salary_model->getEmployeePayroll($id);
	...
	public function sheet(){
		...
		foreach($results as $key=>$result){
			...
			if(isset($branch_info->envirnment)){
				...
				if($envirnment['salary_without_wo']=="no"){
			...
			//$total_cl=$this->
			if(isset($envirnment['salary_cl'])){
				
			}
			$salary_days=(float)$total_days -(float)$result->absent_days+(float)$result->arrear_days-(float)$result->deduction_days;
			
			$basic=$this->parseFormula("basic",$filter);
			$basic=($basic*(float)$salary_days)/(float)$total_days;
			$filter->basic=$basic;
			...
			if($result->site_attendance_check){
				$sitesattendance=$this->salary_model->getSiteAttendanceSalaries($result->atttenace_id,$result->designation_id);
				//printr($sitesattendance);
				$spresent_days=$spwo=$sarrear_day=$sdeduction_day=$sot=0;
				
				foreach($sitesattendance as $site){
					$ssalary_days=$site['present_days']+$site['pwo']+$site['arrear_days']+$site['ot']-$site['deduction_days'];
					//echo $ssalary_days."-".$site['salary']."<br>";
					$sperday_salary=$site['salary']/(float)$total_days;
					//echo $sperday_salary."<br>";
					$ssalary=$sperday_salary*$ssalary_days;
					$sitedata[]=array(
			...
			//exit;
			$net_salary=$gross-$total_deduction-$gratuity-$pt-$total_advance-$total_loan;
	...
	public function sheetwo(){
		...
		foreach($results as $key=>$result){
			...
			//exit;
			$net_salary=$gross-$total_deduction-$gratuity-$pt;
	...
	public function process(){
		...
		$data['heading_title'] 	= 'Salary Process for '. date('F, Y', strtotime($date));
		$filter_data = array(
			'branch' => $data['branch_id'],
            'month'  	=> $data['month']
		);
		$employees_salaries=$this->salary_model->getSalarySheets($filter_data);
        $data['sheets']=[];
		foreach($employees_salaries as $key=>$employee){
		...
		$this->template->view('salaryProcess',$data);
	}
    public function payment(){
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST' && $this->validatePaymentForm()){
            ...
            $salary=$this->getPayroll($user_id,$month);
            $salary=array_merge($salary,$_POST);
            
            $userid=$this->payment_model->addPayment($salary);
        ...
        //exit;
        foreach($this->salary_model->getTableColumnsByName("salary_payment") as $field) {
            if($this->input->post($field)) {
                $data[$field] = $this->input->post($field);
        ...
        $data['salary_days']=(float)$data['month_days'] -(float)$data['absent_days']+(float)$data['arrear_days']-(float)$data['deduction_days'];

        $filter_data = array(
            'month'  	=> $month,
            'user_id'   => $user_id
        );
        $result=$this->salary_model->getSalarySheets($filter_data);

        $data['salary']=$this->getPayroll($result,$month);
        //printr($debit);
        //printr($credit);
        //exit;
        $this->template->view('salaryPayment',$data);
    }
    public function slip(){
        ...
        $this->template->view('salarySlip',$data);
    }

    private function getSalary($user_id,$month){

        $result =$filter= $this->salary_model->getSalarySheet($user_id,$month);
        ...
        if($result->site_attendance_check){
            ...
            foreach($sitesattendance as $site){
                ...
                $credit[$site['name']]=$ssalary;
            }

        }
        $ot_amount=$credit['OT']=$perday_nsalary*((float)$result->holidays+(float)$result->ot+(float)$result->pwo);
    ...
    public function getPayroll($result,$month){
        ...
        if(isset($branch_info->envirnment)){
            $envirnment = json_decode($branch_info->envirnment,true);
            //printr($envirnment);
            if(isset($envirnment['salary_without_wo']) && $envirnment['salary_without_wo']=="no"){
                $total_days=(float)$result->month_days-(float)$result->weekly_off;
            }else{
                $total_days=(float)$result->month_days;
            }
		...
		//echo $basic;
		//echo $total_days;
        $basic=$total_days?($basic*(float)$salary_days)/(float)$total_days:0;
      //echo $basic;
	  //exit;
		$filter->basic=$basic;
        $perday_salary=$total_days?$basic/(float)$total_days:0;
        $perday_nsalary=$salary_days?(float)$result->ctc/(float)$salary_days:0;
